...
//reference velocity is set to 40KMPH
double ref_v = 40;

size_t x_start = 0;
size_t y_start = x_start + 1;
size_t psi_start = y_start + 1;
size_t v_start = psi_start + 1;
size_t cte_start = v_start + 1;
size_t epsi_start = cte_start + 1;
size_t delta_start = epsi_start + 1;
size_t a_start = delta_start + 1;
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // cost funcn based on reference state
    for(int t=0; t < N; ++t){
...
//
    ...
    for (int t = 1; t < N; ++t) {
      ...
      AD<double> f0 = coeffs[0] + coeffs[1] * x0 + coeffs[2]*x1;
...
std::vector<double> MPC::Solve(const VectorXd &state, const VectorXd &coeffs) {
  ...
  // CHECK -> set the initial variable values
  vars[x_start] = x;
  vars[y_start] = y;
  vars[psi_start] = psi;
  vars[v_start] = v;
